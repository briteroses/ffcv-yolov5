...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, ffcv_path, num_workers = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.ffcv_path, opt.num_workers
    ...
    train_loader = load_ffcv_dataset(ffcv_path, 'train', batch_size, imgsz, num_workers)
    '''
    ...
    # Process 0
    if RANK in [-1, 0]:
        val_loader = load_ffcv_dataset(ffcv_path, 'val', batch_size // WORLD_SIZE * 2, num_workers)
        '''
